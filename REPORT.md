...
correctness oracle (`test_correctness.nix`) checks every key against
`fromJSON`.
...
`build_nkv.py INPUT.json --shards {16,256,4096} --prefix DIR/ [--check]`
...
| `nkv-table.nix` | the 255-entry base-255 decode table (generated by `build_nkv.py --write-table`; a format constant) |
| `build_nkv.py` | JSON → nkv builder (single file or `--shards/--prefix`) with independent parser + `--check` |
...
```sh
python3 build_nkv.py INPUT.json OUTPUT.nkv --check        # single file
python3 build_nkv.py INPUT.json --shards 256 --prefix DIR/ --check   # sharded
python3 build_nkv.py --write-table nkv-table.nix          # regenerate the static table
```
...
`test_correctness.nix` (every key, plus a known miss, plus `count` and
...
Harness: `benchmarks/bench.py` (parent), `multiverse-faster/bench.py`
(real workload); raw output: `benchmarks/bench_results.json` and
...
from `benchmarks/bench_marginal.json`; 200k from `benchmarks/bench_results.json`).
...
8. **The static table must sit next to `nkv.nix`** (imported by relative
   ...
   content — regenerated by `build_nkv.py --write-table`.
...
nix eval --impure --json --expr '(import ./test_correctness.nix) "small"'
nix eval --impure --json --expr '(import ./test_correctness.nix) "medium"'
nix eval --impure --json --expr '(import ./test_correctness.nix) "large"'
