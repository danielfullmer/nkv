...
Usage: python3 build_nkv.py input.json output.nkv [--check]
...
4096 shards; disjoint from the probe-seed slice 56..64):
  ...
  python3 build_nkv.py input.json --shards 256 --prefix dir/ [--check]
...
(digit in 0..254 -> byte in 1..255, big-endian digits), so every byte in
...
< 255^3 (widths capped at 3 bytes); M and keyOff < 255^4 (widths
...
B = 255
MAX_FIELD3 = B ** FL - 1             # 16,581,374
MAX_FIELD4 = B ** FO - 1             # 4,228,250,624
...
def main():
    ...
    if not args.input:
        ap.error("input JSON object required")
