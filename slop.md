...
At high lookup counts the advantage narrows: `fromJSON`'s parse-once/index-many model still wins bulk scans (measured with the pre-fix reader: 0.38 s vs 0.89 / 2.49 s at 31,904 lookups/eval on the nixpkgs-multiverse data — the sharded figure predates the open-once reader, which removes the per-lookup shard re-read; see Performance). nkv is for the common case of one or a few lookups per eval, where it is 7.6–146× faster on the data work.
...
- The reader (`nkv.nix`) takes `digits` + `dir`, computes the shard name, and reads that shard file — lazily, and at most once per eval: the module holds one shared `open` thunk per shard name (Nix's call-by-need sharing; `builtins.readFile` is not memoized by path), so repeated lookups on the same shard reuse the first read and a single lookup touches exactly one shard file.
...
- In the sharded layout, `sha256(key)[24:26]` selects the shard file first (one lazy `open`, read at most once per eval); the same probe runs inside it.
...
- Sharded: `import ./nkv.nix { digits = 2; dir = ./data/large_shards; }` — `digits` selects which `sha256` hex slice names the shard (1 → 16, 2 → 256, 3 → 4096 shards; must match what the builder was given) and `dir` is the shard directory. Lookup is lazy — no shard file is read until a lookup forces one, and each shard is read at most once per eval (the module holds one shared `open` thunk per shard name; `readFile` is not memoized by path, so that shared thunk is what makes repeated lookups on the same shard free). `db.count` forces **every** shard (it is the sum of the per-shard header `N` fields) — use it offline, not in a hot path.
...
- **The intercept, not the slope, is where nkv wins.** `fromJSON` pays its readFile + parse once (~173–181 ms on 200k; 121–127 versions; 221–228 history) and then lookup is nearly free within the eval. nkv pays ~0.1–0.5 ms per lookup in-process, plus the per-eval file cost (one 13.7 MB read for the single file; one read per distinct shard for the sharded version, at most once each).
- **Sharding wins the low-query regime.** nkv-sharded pays one small-shard readFile per *distinct* shard (~0.06 ms each, read at most once per eval) instead of a 13.7 MB readFile: at 1 lookup its **work is ~1.2 ms** (multiverse: ≈0–3 ms) — the ~34 ms total sits at the measured 33.0 ms Nix load floor (23.2–38.7) — ≈146× the data work of `fromJSON` on 200k keys (1.2 vs 174.7 ms); at n = 200 the work has risen to 7.5–19.5 ms (200k), 16.5–18.7 ms (versions), 19.2–23.4 ms (history) as queries spread across the 256 shards, ~0.03–0.12 ms/lookup. Once every shard is touched (n ≥ 256) the marginal cost drops to the ~0.017 ms probe: on the 200k table the open-once reader does 10,000 lookups in 211 ms where the pre-fix reader took 836 ms (`benchmarks/many/` sweep).
- **Crossover.** On the multiverse tables nkv-sharded is ahead through n = 100 (versions 42.5 vs 46.1 total, work 10.1 vs 15.2; history 44.0 vs 49.3, work 10.6 vs 19.5) and single-file takes over at n = 200 (versions 44.2 vs 50.1, work 11.1 vs 16.5; history 49.7 vs 51.7, work 16.2 vs 19.2) — the crossover is between ~100 and ~200 lookups/eval (measured with the pre-fix reader; the open-once reader removes the per-lookup shard re-read, so the crossover moves to higher n). On the 13.7 MB 200k table nkv-sharded is ahead across the whole measured range (at n = 200: 46.2 vs 59.3 ms min row, work 7.5 vs 26.0; 52.9 vs 63.8 median, work 19.5 vs 32.2) — and still ahead at 10,000 lookups (211 vs 223 ms, open-once reader).
- **Bulk scans tip back to `fromJSON`.** When one evaluation touches most of the table, parse-once/index-many wins: a 31,904-lookup scan of all values (serialized) runs in 0.38 s with `fromJSON` vs 0.89 / 2.49 s with nkv / nkv-sharded (the sharded figure was measured with the pre-fix reader, which re-read the shard on every lookup; with the open-once reader a full scan costs one read per shard plus the ~0.017 ms probe per lookup, so expect a large improvement when re-measured).
...
- **`builtins.fromJSON` still wins for bulk scans** — if an evaluation touches most of the table, parsing once and indexing the attrset beats per-lookup file slicing (measured with the pre-fix reader: 31,904-lookup scan 0.38 s vs 0.89 / 2.49 s; the sharded figure predates the open-once reader). The crossover is beyond 200 lookups/eval on the parent tables and at ~100–200 for sharded vs single-file on the multiverse tables (pre-fix; the open-once reader removes the per-lookup shard re-read); at ~1k entries all approaches tie at ~34 ms total, of which ~0–1 ms is data work — the rest is Nix itself loading. nkv targets the common case: one or a few lookups per eval.
...
- **Memory** — the index is never parsed into a Nix data structure; each lookup allocates a constant number of small heap strings (probe-seed, key/value fragments). The file string itself is materialised per read: the sharded variant reads each distinct shard at most once per eval (~11–50 KB each; all 256, ≈ the whole file, for `db.count` or a scan that touches most shards), the single-file variant reads the whole table.
